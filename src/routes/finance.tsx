...
import { api, type FinanceResult } from "@/lib/api";
...
function Delta({ pct, inverse = false }: { pct: number; inverse?: boolean | undefined }) {
  const up = pct >= 0;
...
const FINANCE_FALLBACK: FinanceResult = {
  exposure_idr: FINANCE[0]!.value,
  loss_avoided_idr: FINANCE[1]!.value,
  recovery_spend_idr: FINANCE[2]!.value,
  net_benefit_idr: FINANCE[3]!.value,
  weekly_trend: WEEKLY_FINANCE.map((w) => ({
    label: w.label,
    lossAvoided: Math.round(w.lossAvoided / 1_000_000),
    recoveryCost: Math.round(w.recoveryCost / 1_000_000),
  })),
  action_breakdown: [],
...
function FinancePage() {
  const { data: finance, loading, isEmpty } = useApiData(FINANCE_FALLBACK, api.finance);
  ...
  const kpis = [
    {
      ...
      value: formatRp(finance.exposure_idr ?? (liveExposure || FINANCE[0]!.value)),
      delta: -12,
      sub: `${finance.open_exception_count} open exception${finance.open_exception_count !== 1 ? "s" : ""}`,
      inverseGood: true,
    },
    {
      label: "Loss avoided (30d)",
      value: formatRp(finance.loss_avoided_idr ?? (FINANCE[1]!.value + liveAvoided)),
      delta: +14,
      sub: "AI-recommended actions approved",
    },
    {
      label: "Recovery spend (30d)",
      value: formatRp(finance.recovery_spend_idr ?? FINANCE[2]!.value),
      delta: +8,
    ...
    },
    {
      ...
      value: formatRp(finance.net_benefit_idr ?? (FINANCE[3]!.value + liveAvoided)),
      delta: +16,
      sub: "Loss avoided minus recovery spend",
    },
  ];

  // Weekly trend in Rp millions for chart
  const weeklyChartData = finance.weekly_trend.length > 0
    ? finance.weekly_trend
    : WEEKLY_FINANCE.map((w) => ({
        label: w.label,
        lossAvoided: Math.round(w.lossAvoided / 1_000_000),
        recoveryCost: Math.round(w.recoveryCost / 1_000_000),
      }));
            ...
            <div className="panel overflow-hidden">
              ...
              <div className="p-4" style={{ height: 220 }}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyChartData} barCategoryGap="30%" barGap={2}>
